    ...
    public function update(Request $request)
    {
        ...
        try {
            ...
            if ($currentData) {
                ...
                // check delete flag: [name ex: logo_delete]
                if ($request->get('logo_delete') != null) {
                    $new['logo'] = null; // filename for db

                    if ($currentData->{'logo'} != 'logo.png') {
                        @unlink(Config::get('const.UPLOAD_LOGO_PATH') . $currentData['logo']);
                    }

                    if ($currentData->{'favicons'} != 'favicon-96x96.png') {
                        @unlink(Config::get('const.UPLOAD_FAV_PATH') . $currentData['favicons']);
                    }
                }

                // if new logo is being uploaded
                // upload logo
                if ($request->hasFile('logo')) {
                    $file = $request->file('logo');
                    // logo file name example: [id]_logo.jpg
                    ${'logo'} = $currentData->id . "_logo." . $file->getClientOriginalExtension();
                    ${'fav'} = $currentData->id . "_fav." . $file->getClientOriginalExtension();
                    $new['logo'] = ${'logo'};
                    $new['favicons'] = ${'fav'};
                    // save logo and fav to the path
                    $pathLogo = Config::get('const.UPLOAD_LOGO_PATH');
                    $pathFav = Config::get('const.UPLOAD_FAV_PATH');

                    $file->move($pathLogo , ${'logo'});
                    \File::copy($pathLogo . ${'logo'}, $pathFav . ${'fav'});

                    // Resize photo when upload
                    $toolsController = new ToolsController();
                    $toolsController->resizePhoto($pathLogo . $new['logo'], 196);
                    // Fav
                    $toolsController->resizePhoto($pathFav . $new['favicons'], 96);
                }else{
                    if ($request->get('logo_delete') != null) $new['logo'] = 'logo.png';
                    if ($request->get('logo_delete') != null) $new['favicons'] = 'favicon-96x96.png';
                }
